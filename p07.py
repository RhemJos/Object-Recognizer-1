...
confidence_threshold = 0.84  # En porcentaje, ejemplo: 50% como mínimo
...
dog_class_id = [key for key, value in model.names.items() if value == 'person'][0]
...
for box in detections.boxes:
    ...
    if int(box.cls) == dog_class_id and confidence >= confidence_threshold:
...
cv2.imshow("Personas Detectadas con Confianza >= Umbral", image)
cv2.waitKey(0)
